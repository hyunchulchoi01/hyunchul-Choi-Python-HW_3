...
def create_diamonds(symbol):            
  Square1 = Square(RADIUS * 3) 
  Square1.setFillColor('red')
  Square1.setBorderWidth(0)
  Square1.rotate(45)
  symbol.add(Square1) 
  ## 5. create diamonds
...
class CardGraphics(object):
  ...
  def __init__(self, card, hidden = False):
    ...
    if card.suit == "Clubs":
      create_clubs(symbol)
    elif card.suit == "Diaminods": 
      create_diamonds(symbol)
    elif card.suit == "Haearts": 
...
class Hand(object):                    ########## 테이블에서 보여지는 카드들 (player 별로)
  ...
  def add(self, card, hidden = False):
    ...
    grap = CardGraphics(card, hidden)
    grap.l.moveTo(self.x + CARD_SIZE[0] * 2 * len(self.graphics), self.y)
    self.canvas.add(grap.l)
    self.graphics.append(grap)
    # 11.
